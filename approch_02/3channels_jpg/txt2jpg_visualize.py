...
def main():
    file_path = "dataset\\transition_txt\\Tile6.txt"
    ...
    inten_principle_value = intensity_principal_gradient_positions(data)
    
    resolution = 0.2
    occupancy_grid_with_intensity = create_occupancy_grid_with_color_and_intensity(data, resolution, grayscale_color, normalized_intensity_values, inten_principle_value)
    print("Occupancy Grid Shape:", occupancy_grid_with_intensity.shape)


    #Visualize the occupancy channel
    plt.subplot(131)
    plt.title('intensities')
    plt.imshow(occupancy_grid_with_intensity[:, :, 0], cmap='jet', origin='lower')
    
    #Visualize the grayscale color channel
    plt.subplot(132)
    plt.title('Grayscale Color Channel')
    plt.imshow(occupancy_grid_with_intensity[:, :, 1], cmap='jet', origin='lower')
    
    #Visualize the normalized intensity channel
    plt.subplot(133)
    plt.title('principle')
    plt.imshow(occupancy_grid_with_intensity[:, :, 2], cmap='jet', origin='lower')
